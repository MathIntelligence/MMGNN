...
from mmgnn.utils import makedirs
from mmgnn.features import get_available_features_generators
...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--save_dir', type=str, default=None,
                        help='Directory where model checkpoints will be saved. '
                             'If not provided, defaults to {dataset}/{mode}.')
    ...
    # Pipeline mode: 2D vs 3D 
    parser.add_argument('--mode', type=str, default='3d',
                        choices=['2d', '3d'],
                        help='Training pipeline: 2d = 2D graphs (SMILES + optional atom features), 3d = 3D graphs (mol2 coordinates, RBF/angular bond features)')
    parser.add_argument('--agg', type=str, default='sum',
                        choices=['sum', 'mean', 'attn'],
                        help='Local subgraph aggregation: sum, mean, or attn')
...
def modify_train_args(args: Namespace):
    ...
    args.global_mode = False
    args.local_mode = True
    args.baseline_full = False
    args.recon_only = True
    args.recon_pool = getattr(args, 'agg', 'sum')
    args.dual_agg = 'sum'

    # Set save directory based on pipeline mode (2d/3d) and dataset name (local mode only)
    if args.save_dir is None:
        # Extract dataset name from data_path
        dataset_name = os.path.splitext(os.path.basename(args.data_path))[0]
        pipeline_mode = getattr(args, 'mode', '3d')
        save_suffix = f'local_{args.recon_pool}'
        args.save_dir = os.path.join('results', 'model', dataset_name, pipeline_mode, save_suffix)
