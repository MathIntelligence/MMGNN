...
from mmgnn.data import MoleculeDataset
from mmgnn.nn_utils import compute_gnorm, compute_pnorm, NoamLR
...
def train(model: nn.Module,
    ...
    for i in trange(0, num_iters, iter_size):
        ...
        # Prepare subgraphs for local reconstruction
        batched_sub = None
        sub_to_mol = None
        from mmgnn.features.subgraph import batch_subgraphs
        from mmgnn.features import mol2graph
        ...
        if batched_sub is not None and sub_to_mol is not None:
            preds = model(batch, features_batch, batched_sub, sub_to_mol)
        else:
