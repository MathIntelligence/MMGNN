...
from mmgnn.features import get_features_generator
...
class MoleculeDatapoint:
    ...
    def __init__(self,
                 ...
                 atom_coordinates: Optional[List[Tuple[float, float, float]]] = None):
        """
        ...
        :param features: A numpy array containing additional molecule-level features (ex. Morgan fingerprint).
        :param use_compound_names: Whether the data CSV includes the compound name on each line.
        :param atom_features: (2D mode) Per-atom external features, shape (n_atoms, feature_dim).
        :param mol2_path: (3D mode) Path to the mol2 file containing 3D coordinates.
        :param atom_coordinates: (3D mode) List of (x, y, z) tuples for each atom's 3D coordinates.
        """
        if args is not None:
        ...
        # Store 3D molecular information
        self.mol2_path = mol2_path
        self.atom_coordinates = atom_coordinates  # List of (x, y, z) tuples
        ...
        # Store subgraphs (will be populated during data loading if subgraph mode is enabled)
        self.subgraphs = None  # List of SimpleMolGraph2D objects
...
class MoleculeDataset(Dataset):
    ...
    def atom_features(self) -> List[np.ndarray]:
        """
        Returns the atom-level external features associated with each molecule (if they exist).
        Used in 2D mode when features_path provides per-atom features.

        :return: A list of 2D numpy arrays (n_atoms x feature_dim) or None if there are no atom features.
        """
        if len(self.data) == 0 or self.data[0].atom_features is None:
            return None
        return [d.atom_features for d in self.data]

    def normalize_atom_features(self, scaler: StandardScaler = None, replace_nan_token: float = 0.0) -> Optional[StandardScaler]:
        """
        Normalizes atom-level features using a StandardScaler (2D mode with external atom features).
        If a scaler is provided, uses that scaler. Otherwise fits a scaler on this dataset.

        :param scaler: A fitted StandardScaler. If provided, uses this scaler. Otherwise fits a new scaler.
        :param replace_nan_token: Value to replace NaN with before normalization (default: 0.0).
        :return: The fitted StandardScaler, or None if no atom features.
        """
        if len(self.data) == 0 or self.data[0].atom_features is None:
            return None
        atom_features_list = self.atom_features()
        cleaned_features = []
        for mol_feats in atom_features_list:
            mol_feats_clean = mol_feats.copy()
            mol_feats_clean[np.isnan(mol_feats_clean)] = replace_nan_token
            cleaned_features.append(mol_feats_clean)
        if scaler is not None:
            self.atom_scaler = scaler
        elif self.atom_scaler is None:
            all_atoms = np.vstack(cleaned_features)
            self.atom_scaler = StandardScaler(replace_nan_token=replace_nan_token)
            self.atom_scaler.fit(all_atoms)
        for i, mol_feats in enumerate(cleaned_features):
            normalized_mol = self.atom_scaler.transform(mol_feats)
            self.data[i].atom_features = normalized_mol
        from mmgnn.features import clear_cache
        clear_cache()
